...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const manageGuestsButtonWrapper = document.getElementById('manageGuestsButtonWrapper'); // Div wrapper do botão
    const manageGuestsBtn = document.getElementById('manageGuestsBtn'); // Botão Gerenciar Convidados
    const guestManagementModal = new bootstrap.Modal(document.getElementById('guestManagementModal')); // Modal Gerenciar Convidados
    const addGuestForm = document.getElementById('addGuestForm'); // Formulário de adicionar convidado
    const guestNameInput = document.getElementById('guestName'); // Input Nome do Convidado
    const guestEmailInput = document.getElementById('guestEmail'); // Input Email do Convidado
    const addGuestBtn = document.getElementById('addGuestBtn'); // Botão de adicionar convidado no formulário
    const guestsList = document.getElementById('guestsList'); // UL para a lista de convidados
    ...
    async function loadEventDetails() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // --- NOVO: Lógica de visibilidade do botão 'Gerenciar Convidados' (para ADMIN ou Dono do Evento) ---
                currentEventOwnerEmail = event.eventOwnerEmail; // Este campo virá do EventoResponseDto
                const loggedInUserEmail = getUserEmail(); // Email do usuário logado

                if (userRole === 'ADMIN' || (currentEventOwnerEmail && loggedInUserEmail && currentEventOwnerEmail === loggedInUserEmail)) {
                    manageGuestsButtonWrapper.style.display = 'block'; 
                } else {
                    manageGuestsButtonWrapper.style.display = 'none';
                }
                // --- Fim da lógica do botão 'Gerenciar Convidados' ---
    ...
    async function loadGuestsForEvent(eventId) {
        ...
        try {
            ...
            if (response.ok) {
            ...
            } else if (response.status === 204 || response.status === 404) { // 204 No Content se não houver convidados ou 404 se o evento não for encontrado
                ...
                removeToken();
                // Não redireciona aqui, pois o modal está aberto, mas o usuário não poderá interagir
            } else {
                const errorData = await response.json().catch(() => ({ message: 'Erro desconhecido' }));
